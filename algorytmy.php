<?php 
    ...
    require './php/functionality/db.php';
        if(!isset($_GET['name']) || !isset($_GET['id'])) {
            $query = "SELECT name, id FROM lists where type = 'algorytmy' LIMIT 1";
...
<main class="main posts">
    <section class="excercises">
        <div class="excercises-wrap">
            <h2 class="excercises-wrap_name h2">algorytmy</h2>
            <?php   
                $query = "SELECT name FROM lists WHERE type = 'algorytmy'";
                $results = $connection->query($query);
            ...
            <script>
                ...
                select.addEventListener("change", ()=>location.replace(`./algorytmy.php?${select.value}`))
            </script>    
        </div>
    ...
    <aside class="sidebar">
        <nav class="sidebar-navigation">  
            ...
            <?php
                $query = "SELECT id, name FROM lists WHERE type = 'algorytmy'";
                $results = $connection->query($query);
                while($row = mysqli_fetch_assoc($results)){
                    echo '<li><a class="sidebar-navigation_list--link h5" href="./algorytmy.php?name='.$row['name'].'&id='.$row['id'].'#">'.$row['name'].'</a></li>';
                } 
            ?>
            </ul>
        </nav>
        <nav class="sidebar-navigation">  
            ...
            <?php
                ...
                if (!mysqli_stmt_prepare($init, $query)) {
                    ...
                    $results = mysqli_stmt_get_result($init);
                    while($row = mysqli_fetch_assoc($results)){
                        echo '<li><a class="sidebar-navigation_list--link h5" href="./algorytmy.php?name='.$row['name'].'#'.$row['title'].'">'.$row['title'].'</a></li>';
                    }
